...
def create_top_file(module_name,reg_model):
    ...
    top_file.write('    ral_block_%s_reg_model reg_model;\n'%(module_name))
    top_file.write('\n')   
    ...
    top_file.write('        uvm_config_db#(ral_block_%s_reg_model)::set(null, "*","reg_model",reg_model);\n'%(module_name))
...
def create_base_test_file(module_name):
    ...
    test_file.write('        phase.phase_doone.set_drain_time(this,1000);\n')
...
def create_example_test_file(module_name):
    ...
    example_test_file.write('class %s_example_test extends uvm_test;\n'%(module_name))
...
def create_base_seq_file(module_name):
    ...
    seq_file.write('    ral_block_%s_reg_model reg_model;\n'%(module_name))
    ...
    seq_file.write('        if(!uvm_config_db#(ral_block_%s_reg_model)::get(null, "","reg_model",reg_model))\n'%(module_name))
...
def create_example_seq_file(module_name):
    ...
    example_seq_file.write('class %s_example_seq extends uvm_sequence#(%s_transaction);\n'%(module_name,module_name))
    example_seq_file.write('\n')     
    ...
    example_seq_file.write('        //Add You')
    example_seq_file.write('\n')
    example_seq_file.write('\n')
    example_seq_file.write('\n')
    example_seq_file.write('\n')
...
def create_env_file(module_name,reg_model):
    ...
    env_file.write('\n')
    env_file.write('    ral_block_%s_reg_model reg_model;\n'%(module_name))
    env_file.write('\n')
    ...
    env_file.write('        if(!uvm_config_db#(ral_block_%s_reg_model)::get(this, "","reg_model",reg_model))\n'%(module_name))
    ...
    env_file.write('\n')
    env_file.write('        ref_mdoel = %s_ref_model::type_id::create("ref_mdoel", this);\n'%(module_name))
    env_file.write('\n')    
    if(reg_model == 'apb'):
...
def create_scb_file(module_name):
    ...
    scb_file.write('\n')
    scb_file.write('    ral_block_%s_reg_model reg_model;\n'%(module_name))
    ...
    scb_file.write('        if(!uvm_config_db#(ral_block_%s_reg_model)::get(this, "","reg_model",reg_model))\n'%(module_name))
...
'''
    ...
    makefile.write('\n')
    makefile.write('other_file := *.h simv.* *.log csrc *.key *.vpd DVEfiles* coverage *.vdb verdiLog novas* *fsdb* _vcs* *signal* *tmp* core*\n')
    ...
    makefile.write('\t@echo "---------- ntb_random_seed = $(seed)" >> ./log/$(case)_log/$(case)_comp.log\n')
    makefile.write('\n')
    ...
    makefile.write('\tverdi -f filelist.f -ssf ./wave/$(case_name)_wave/*$(fsdb_file).fsdb -sv -nologo &\n')
    makefile.close()
